/// Auto-generated unreflector glue for LogicalFunction::Add.
...
namespace
{
const auto registered_Add_LogicalFunction = [] {
    const bool registered = LogicalFunctionUnreflectionRegistry::instance().addUnreflectorEntry(
        ...
        [](const Reflected& data, const ReflectionContext& context) {
            return context.unreflect<AddLogicalFunction>(data);
        });
    if (!registered)
    {
        /// Static-init context: an uncaught throw aborts the program loudly, without main() being able to catch it.
        throw std::runtime_error{std::string{"Duplicate unreflection registration for \"Add\" in LogicalFunction"}};
    }
    return 0;
}();
}

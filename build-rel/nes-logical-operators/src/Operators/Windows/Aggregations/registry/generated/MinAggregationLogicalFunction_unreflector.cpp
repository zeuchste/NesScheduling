/// Auto-generated unreflector glue for AggregationLogicalFunction::Min.
...
namespace
{
const auto registered_Min_AggregationLogicalFunction = [] {
    const bool registered = AggregationLogicalFunctionUnreflectionRegistry::instance().addUnreflectorEntry(
        ...
        [](const Reflected& data, const ReflectionContext& context) {
            return context.unreflect<MinAggregationLogicalFunction>(data);
        });
    if (!registered)
    {
        /// Static-init context: an uncaught throw aborts the program loudly, without main() being able to catch it.
        throw std::runtime_error{std::string{"Duplicate unreflection registration for \"Min\" in AggregationLogicalFunction"}};
    }
    return 0;
}();
}

/// Auto-generated unreflector glue for AggregationLogicalFunction::Max.
...
namespace
{
const auto registered_Max_AggregationLogicalFunction = [] {
    const bool registered = AggregationLogicalFunctionUnreflectionRegistry::instance().addUnreflectorEntry(
        ...
        [](const Reflected& data, const ReflectionContext& context) {
            return context.unreflect<MaxAggregationLogicalFunction>(data);
        });
    if (!registered)
    {
        /// Static-init context: an uncaught throw aborts the program loudly, without main() being able to catch it.
        throw std::runtime_error{std::string{"Duplicate unreflection registration for \"Max\" in AggregationLogicalFunction"}};
    }
    return 0;
}();
}

...
void EmitPhysicalOperator::open(ExecutionContext& ctx, RecordBuffer& inputRecordBuffer) const
{
    ...
    /// smallest fitting class, with them off it returns the default-size buffer. EagerFull keeps the original full-size
    /// allocation. ReuseAcrossRuns resumes into the partial buffer carried from the previous invocation (see below).
    /// Output buffer under-estimates are handled by the flush-on-full path in execute().
    /// TODO(#1711): StageAndCopy = copy to an exact buffer at flush.
...
void EmitPhysicalOperator::close(ExecutionContext& ctx, RecordBuffer&) const
{
    ...
    if (mode == EmitBufferAllocationMode::ReuseAcrossRuns && !ctx.lastChunk)
    {
        ...
        /// amortising the allocation. Flush still happens at the sequence boundary (the lastChunk branch below) or, as a
